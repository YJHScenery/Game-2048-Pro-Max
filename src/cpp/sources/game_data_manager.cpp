...
GameData GameDataManager::loadGame(const QList<uint64_t> &mode) const
{
    ...
    record.maxScore = 0;
    // 初始化为空棋盘，长度应与模式维度乘积一致
    qsizetype cellCount = 1;
    if (mode.isEmpty())
    {
        cellCount = 4;
    }
    ...
    {
        for (const auto dim : mode)
        {
            cellCount *= static_cast<qsizetype>(dim > 0 ? dim : 1);
        }
    }
    record.flatTensorData = QList<std::uint64_t>(cellCount, 0);
