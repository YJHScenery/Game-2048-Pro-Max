/**
 * @file ai_mode.c
 * @brief 2048 AI 启发式评估与 Alpha-Beta 搜索实现。
 * @details
 * 本文件实现启发式打分、对抗/环境层搜索，以及用于 AI 演示的控制台输出辅助函数。
 */
#include <cmath>
#include <cstdio>
#include <cfloat>
...
float calculate_monotonicity(const Board* board, int boardsize)
{
    ...
    for (int i = 0; i < boardsize; i++) {
        ...
        while (next < boardsize) {
            while (next < boardsize && board->board[i][next] == 0)
                next++;
            if (next >= boardsize)
                next--;
    ...
    for (int j = 0; j < boardsize; j++) {
        ...
        int next = current + 1;
        while (next < boardsize) {
            while (next < boardsize && board->board[next][j] == 0)
                next++;
            if (next >= boardsize)
                next--;
...
AI2048Result search_best(Board* this_board, int depth, float alpha, float beta, int playerTurn, int boardsize)
{
    ...
    if (playerTurn) {
        ...
        auto* score_2 = (float*)malloc(freeCount * sizeof(float));
        auto* score_4 = (float*)malloc(freeCount * sizeof(float));
...
int getBestMove(Board* board, int boardsize)
{
    ...
    while (depth > 0) {
        if (result.move == -1)
            result = search_best(board, --depth, -1000000.0f, 1000000.0f, 1, boardsize);
        else
            break;
    }
    if (result.move == -1)
...
void print_board_fastest(Board* board, int boardsize)
{
    ...
    case 6:
        printf(" ------- ------- ------- ------- ------- ------- \n"
               ...
               board->board[4][0], board->board[4][1], board->board[4][2], board->board[4][3], board->board[4][4],
               board->board[4][5],
               board->board[5][0], board->board[5][1], board->board[5][2], board->board[5][3], board->board[5][4],
               board->board[5][5]);
        break;
    case 8:
        printf(" ------- ------- ------- ------- ------- ------- ------- ------- \n"
               ...
               " ------- ------- ------- ------- ------- ------- ------- ------- \n",
               board->board[0][0], board->board[0][1], board->board[0][2], board->board[0][3], board->board[0][4],
               board->board[0][5], board->board[0][6], board->board[0][7],
               board->board[1][0], board->board[1][1], board->board[1][2], board->board[1][3], board->board[1][4],
               board->board[1][5], board->board[1][6], board->board[1][7],
               board->board[2][0], board->board[2][1], board->board[2][2], board->board[2][3], board->board[2][4],
               board->board[2][5], board->board[2][6], board->board[2][7],
               board->board[3][0], board->board[3][1], board->board[3][2], board->board[3][3], board->board[3][4],
               board->board[3][5], board->board[3][6], board->board[3][7],
               board->board[4][0], board->board[4][1], board->board[4][2], board->board[4][3], board->board[4][4],
               board->board[4][5], board->board[4][6], board->board[4][7],
               board->board[5][0], board->board[5][1], board->board[5][2], board->board[5][3], board->board[5][4],
               board->board[5][5], board->board[5][6], board->board[5][7],
               board->board[6][0], board->board[6][1], board->board[6][2], board->board[6][3], board->board[6][4],
               board->board[6][5], board->board[6][6], board->board[6][7],
               board->board[7][0], board->board[7][1], board->board[7][2], board->board[7][3], board->board[7][4],
               board->board[7][5], board->board[7][6], board->board[7][7]);
        break;
    default:
        break;
    }
